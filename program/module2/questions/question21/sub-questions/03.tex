...
\begin{proof}
    Выберем произвольный вектор $\vec{x} = x_1\vec{b}_1 + \ldots + x_n\vec{b}_n$. Его образом будет вектор

    \begin{align*}
        \vec{y} &= \mathscr{A}\vec{x} =  \mathscr{A}(x_1\vec{b}_1 + \ldots + x_n\vec{b}_n) = x_1(\mathscr{A}\vec{b}_1) + \ldots + x_n(\mathscr{A}\vec{b}_n) = \\
        &= x_1(a_{11}\vec{b}_1 + \ldots + a_{n1}\vec{b}_n) + \ldots + x_n(a_{1n}\vec{b}_1 + \ldots + a_{nn}\vec{b}_n) = \\
        &= (a_{11}x_1 + \ldots + a_{1n}x_n)\vec{b}_1 + \ldots + (a_{n1}x_1 + \ldots + a_{nn}x_n)\vec{b}_n = \\
        &= \begin{pmatrix}
